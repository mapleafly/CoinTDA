...
public class CoinInfo {
    ...
    /**
     * 更新当前价格数据 从coinmarketcap.com获取coin id 价格等数据
     *
     * @param event
     */
    public boolean updateCurPrice() {
        ...
        List<CoinQuotesLatestBean> list = quotesLatest.getQuotesLatest(k, v);
    ...
    public void handleUpdateCurPrice() {
        if (updateCurPrice()) {
            ...
            workbench.showErrorDialog(
                    "错误",
                    "更新当前价格数据失败！",
                    "请确认网络状况和网站key！/n 请确认是否选择了可用Coin。",
                    buttonType -> {
                    }
            );
        }
    ...
    public void handleUpdateCoinIDMap() {
        if (updateCoinIDMap()) {
            //复写更新时间
            PrefsHelper.updatePreferencesValue(PrefsHelper.COINIDMAP_DATE, DateHelper.toString(LocalDate.now()));
